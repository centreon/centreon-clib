...
void process_manager::add(process* p) {
  ...
  clib_lock_guard<std::mutex> lock(_lock_processes, "add::_lock_processes");
...
process_manager::process_manager()
  ...
  process::_set_close_on_exec(_fds_exit[1]);
...
process_manager::~process_manager() noexcept {
  ...
  {
    clib_lock_guard<std::mutex> lock(_lock_processes, "~process_manager::_lock_processes");
...
void process_manager::_close_stream(int fd) noexcept {
  try {
    ...
    {
      clib_lock_guard<std::mutex> lock(_lock_processes, "_close_stream::_lock_processes");
      _update = true;
...
void process_manager::_kill_processes_timeout() noexcept {
  clib_lock_guard<std::mutex> lock(_lock_processes, "_kill_processes_timeout::_lock_processes");
...
unsigned int process_manager::_read_stream(int fd) noexcept {
  ...
  try {
    ...
    {
      clib_lock_guard<std::mutex> lock(_lock_processes, "_read_stream::_lock_processes");
      std::unordered_map<int, process*>::iterator it(_processes_fd.find(fd));
      if (it == _processes_fd.end()) {
        _update = true;
...
/**
 *  Update process informations at the end of the process. _lock_processes is
 *  already locked.
 *
...
void process_manager::_update_list() {
  clib_lock_guard<std::mutex> lock(_lock_processes, "_update_list::_lock_processes");
  // No need update.
  if (!_update)
    return;
...
void process_manager::_wait_orphans_pid() noexcept {
  try {
    clib_lock_guard<std::mutex> lock(_lock_processes, "_wait_orphans_pid::_lock_processes");
...
void process_manager::_wait_processes() noexcept {
  try {
    for (;;) {
      ...
      clib_lock_guard<std::mutex> lock(_lock_processes, "_wait_processes::_lock_processes");
